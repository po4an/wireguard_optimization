#!/bin/bash
echo -e "Check if wireguard is installed"
wireguard_path=`whereis wireguard | awk -F ": " '{print $2}'`
if [ -n "$wireguard_path" ]; then
	echo -e "Wireguard is installed and on the path: $wireguard_path
    	\nStart setting it up"
	...
	if [ `cat $addresses_data_path | awk -v tech_user_name="$tech_user_name" '$1==tech_user_name {print $1}' | wc -w` != 0 ]; then
		echo -e "The wireguard is already installed and configured"
	else
	...
	echo -e "Wireguard is not installed
		\nTry to update packeges and install it using the command: sudo apt update && sudo apt upgrade -y && sudo apt install -y wireguard
